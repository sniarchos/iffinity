...
"Shown" is meant literally: global scripts run when `showSnippet` puts a snippet on the page, and so do the tag- and snippet-specific scripts below. [`renderSnippet`]({{ site.baseurl }}/author-api/) only hands you a string, so it leaves all of them alone unless you ask for them with its `runScripts` argument. Story scripts are the exception &mdash; they run once at the start of the story whatever asks for the first render.
