...
|`renderSnippet(id, runScripts = false)`|`id` is the name (or numeric id) of a snippet; `runScripts` says whether to also run that snippet's global, tag- and snippet-specific scripts|Renders the snippet with the given id, returning the rendered HTML as a string. By default the scripts are **not** run: they exist to set up a snippet the player is looking at, and they reach for the live document, so running them for a call that only asks for a string would change the visible page behind your back. Pass `true` if you genuinely want them. `showSnippet` always runs them, because that is a real visit, and story code is outside this flag &mdash; it runs once at the start of the story whatever asks for the first render. **Before iffinity 1.0.0 the scripts always ran.**|
...
|`load(data, cb?, landingSnippet?, addToHistory = false)`|`data` is what a previous call to `save()` produced, `cb` is a callback that is called right after restoring the state's story, history and checkpoint, receiving the (restored) `story.state` as its sole argument and returning `void`, `landingSnippet` (if provided) is the name of the snippet to show after a successful load instead of the last snippet in the (restored) history (e.g. a "Load Successful" kind of snippet), and `addToHistory` dictates whether the snippet shown after the load is recorded in the story's history. It is `false` by default, because the restored history already ends where the player was: recording again would either duplicate that entry or write a landing screen over the player's actual position. **Before iffinity 1.0.0 this parameter was called `loadNoHistory` and defaulted to `true`, which meant the opposite of its name;** a story that passed it explicitly should drop the argument or invert it.|Attempts to load the story using the `data` argument, which should be exactly what `save()` produced. Returns nothing.|
